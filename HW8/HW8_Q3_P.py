...
def pool_sulfur_rule(model, pool):
    total_crude_sulfur = sum(model.crude_to_pool[s, pool] * model.supply_sulfur[s] for s in model_P.sources)
    return total_crude_sulfur == model.pool_sulfur[pool] * sum(model.pool_to_product[pool, p] for p in model_P.products)
model_P.pool_sulfur_constraint = Constraint(model_P.pools, rule=pool_sulfur_rule)

# Product Sulfur Constraint
def product_sulfur_rule(model, product):
    return sum(model.pool_sulfur[pool] * model.pool_to_product[pool, product] for pool in model_P.pools) <= model.product_max_sulfur[product] * sum(model.pool_to_product[pool, product] for pool in model_P.pools)
