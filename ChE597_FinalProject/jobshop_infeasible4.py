...
model.downtime = pyo.ConstraintList()
model.downtime.add(model.start['A', 3] + model.dur['A', 3] + 4 <= model.start['B', 2])  # Downtime after A3 before B2
model.downtime.add(model.start['B', 3] + model.dur['B', 3] + 3 <= model.start['C', 1])  # Downtime after B3 before C1

# Introduce slack variables to provide flexibility
model.slack = pyo.Var(['A', 'B'], within=pyo.NonNegativeReals, bounds=(0, None), doc="Slack time for tasks A and B")
...
print("Start times:")
for j, m in model.TASKS:
    print(f"{j}{m}: {pyo.value(model.start[j, m])}")