...
# Task setup with a direct cyclic dependency introduced
Task = {
    ('A', 1): {'dur': 5, 'prec': None},
    ('A', 3): {'dur': 3, 'prec': ('C', 2)},  # C2 must complete before A3 can start
    ('B', 2): {'dur': 3, 'prec': None},
    ('B', 3): {'dur': 2, 'prec': ('B', 2)},
    ('C', 1): {'dur': 2, 'prec': None},
    ('C', 2): {'dur': 4, 'prec': ('A', 3)}  # A3 must complete before C2 can start, creating a cycle with A3
}
...
model.makespan = pyo.Var(bounds=(0, ub-10))
...
# Update the Task dictionary to remove cyclic dependencies
Task[('A', 3)]['prec'] = None  # Remove the precedence from A3 that leads to C2
Task[('C', 2)]['prec'] = None  # Remove the precedence from C2 that leads to A3
