...
def end_cycle_inventory(model):
    return model.inventory[model.T.last()] >= model.initial_inventory + 1500
model.end_cycle_con = pyo.Constraint(rule=end_cycle_inventory, doc='End cycle inventory constraint') # Infeasible on the end cycle inventory.
