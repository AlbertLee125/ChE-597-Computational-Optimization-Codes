...
# Parameters
model.demand = pyo.Param(model.T, initialize={1: 2800, 2: 500, 3: 100, 4: 850}, mutable=True, doc='Demand for each quarter')
model.capacity = pyo.Param(model.T, initialize={1: 1200, 2: 1200, 3: 1200, 4: 1200}, mutable=True, doc='Production capacity for each quarter')
...
model.initial_inventory = pyo.Var(domain=pyo.Reals, doc='Initial Inventory', initialize=-500) # Infeasible on the initial inventory(pre-order scenario).
model.initial_inventory.fix(-500) # Fix the initial inventory to -500 dozens to make the model infeasible.
...
# Constraint to ensure end-cycle inventory is non-negative
def end_cycle_inventory(model):
    return model.inventory[model.T.last()] >= model.initial_inventory
model.end_cycle_con = pyo.Constraint(rule=end_cycle_inventory)
